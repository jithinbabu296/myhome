...
class SaleOrder(models.Model):
    ...
    def woo_convert_dates_by_timezone(self, instance, from_date, to_date):
        ...
        """
        from_date = from_date if from_date else instance.last_order_import_date - timedelta(
            days=1) if instance.last_order_import_date else fields.Datetime.now() - timedelta(days=1)
        to_date = to_date if to_date else fields.Datetime.now()

        from_date = pytz.utc.localize(from_date).astimezone(
            pytz.timezone(instance.store_timezone)) if from_date else False
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        if woo_orders:
            ...
            for instance in instances:
                ...
                res += super(SaleOrder, instance_orders.with_context(round=rounding))._create_invoices(grouped=grouped,
                                                                                                       final=final,
                                                                                                       date=date)
        if orders:
            res += super(SaleOrder, orders)._create_invoices(grouped=grouped, final=final, date=date)
        return res
